...
`--tokenfile`, `--token`, `--netrcfile`, and `--netrc` are mutually exclusive.
Use `--token` to explicitly request token authentication from the
`$BEARER_TOKEN` environment variable (the token value itself is never passed
on the command line):
```
