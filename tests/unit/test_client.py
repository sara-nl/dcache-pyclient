...
class TestConfigPrecedence:
    def test_constructor_api_overrides_env_and_file(self, tmp_path, monkeypatch):
        ...
        tokenfile.write_text("dummy-token\n")
    ...
    def test_env_var_used_when_no_constructor_api_given(self, tmp_path, monkeypatch):
        conf = tmp_path / "ada.conf"
        conf.write_text("api=https://file.example.org/api/v1\n")
        os.chmod(conf, 0o600)
        monkeypatch.setenv("ada_api", "https://env.example.org/api/v1")
    ...
    def test_file_used_when_no_constructor_or_env_api_given(self, tmp_path):
        conf = tmp_path / "ada.conf"
        conf.write_text("api=https://file.example.org/api/v1\n")
        os.chmod(conf, 0o600)
